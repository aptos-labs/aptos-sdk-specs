...
This repository contains language-agnostic behavioral specifications for Aptos SDK implementations. It uses Gherkin (BDD) scenarios to define expected behaviors and JSON test vectors for deterministic validation. The goal is to ensure consistent behavior across all official and community Aptos SDKs.

## Repository Structure

- `features/` - Gherkin `.feature` files organized by category (01-core-types, 02-cryptography, etc.)
...
- Category: `@core-types`, `@cryptography`, `@accounts`, `@transactions`, `@api-clients`, `@advanced`
