...
class TypeTagSteps(private val world: World) {
    ...
    fun givenAStructTagWithComponents(address: String, module: String, name: String) {
    ...
    fun thenTheTypeTagVariantShouldBe(variant: String) {
        val tag = world.typeTag as ParsedTypeTag
        when (variant) {
            "Bool" -> { tag.kind shouldBe "primitive"; tag.value shouldBe "bool" }
            "U8" -> { tag.kind shouldBe "primitive"; tag.value shouldBe "u8" }
            "U16" -> { tag.kind shouldBe "primitive"; tag.value shouldBe "u16" }
            "U32" -> { tag.kind shouldBe "primitive"; tag.value shouldBe "u32" }
            "U64" -> { tag.kind shouldBe "primitive"; tag.value shouldBe "u64" }
            "U128" -> { tag.kind shouldBe "primitive"; tag.value shouldBe "u128" }
            "U256" -> { tag.kind shouldBe "primitive"; tag.value shouldBe "u256" }
            "Address" -> { tag.kind shouldBe "primitive"; tag.value shouldBe "address" }
            "Signer" -> { tag.kind shouldBe "primitive"; tag.value shouldBe "signer" }
    ...
    fun thenTypeArgumentShouldBeStructNamed(index: Int, name: String) {
        val tag = world.typeTag as ParsedTypeTag
    ...
    fun thenTypeArgumentShouldBe(index: Int, expected: String) {
        val tag = world.typeTag as ParsedTypeTag
        val typeArg = tag.typeArgs[index]
        when (expected) {
            "U64" -> { typeArg.kind shouldBe "primitive"; typeArg.value shouldBe "u64" }
            "U8" -> { typeArg.kind shouldBe "primitive"; typeArg.value shouldBe "u8" }
    ...
    private fun parseTypeTag(input: String): ParsedTypeTag {
        ...
        if (match != null) {
            ...
            val typeArgs = if (typeArgsStr.isNotEmpty()) {
