...
/**
 * World class holds test context/state between Cucumber steps. Each scenario
 * gets a fresh World instance via dependency injection.
 *
...
public class World {
	...
	public AptosClient initClient() {
		if (client == null) {
			config = AptosConfig.builder().network(AptosConfig.Network.TESTNET).build();
