...
fn then_initial_delay(world: &mut TestWorld, expected_ms: u64) {
    ...
    assert!(
        diff < 50,
        "Expected initial_delay around {}ms, got {}ms",
        expected_ms,
...
fn then_max_delay(world: &mut TestWorld, expected_secs: u64) {
    ...
    assert!(
        diff < 6000,
        "Expected max_delay around {}s, got {}ms (diff {})",
        expected_secs,
        config.max_delay_ms,
        diff
    );
...
fn then_backoff_exponential(world: &mut TestWorld) {
    let config = world.retry_config.as_ref().expect("No retry config");
    assert!(
        config.exponential_base > 1.0,
        "Exponential base should be > 1.0"
    );
}
...
fn given_retry_config_max_retries(world: &mut TestWorld, max_retries: u32) {
    world.retry_config = Some(RetryConfig::builder().max_retries(max_retries).build());
}
...
fn given_retry_config_backoff(world: &mut TestWorld, factor: f64) {
    world.retry_config = Some(RetryConfig::builder().exponential_base(factor).build());
}
...
fn given_tx_invalid_sequence(world: &mut TestWorld) {
    ...
    world
        .named_values
        .insert("is_retryable".to_string(), "false".to_string());
}
...
fn when_retries_occur(world: &mut TestWorld) {
    ...
    for i in 1..=3 {
        ...
        world
            .named_values
            .insert(format!("delay_{}", i), delay.as_millis().to_string());
    }
}
...
fn when_sdk_makes_request(world: &mut TestWorld) {
    ...
    world
        .named_values
        .insert("attempts".to_string(), (max_failures + 1).to_string());
}

#[then(expr = "it should retry twice")]
fn then_retry_twice(world: &mut TestWorld) {
    let attempts: u32 = world
        .named_values
        .get("attempts")
        .map(|s| s.parse().unwrap())
        .unwrap_or(0);
...
fn given_request_always_fails(world: &mut TestWorld) {
    world
        .named_values
        .insert("always_fails".to_string(), "true".to_string());
}
...
fn then_total_attempts(world: &mut TestWorld, total: u32, retries: u32) {
    ...
    assert_eq!(
        ...
        total,
        "Total attempts should be {} (1 + {} retries)",
        total,
        retries
    );
}
...
fn then_return_final_error(world: &mut TestWorld) {
    ...
    assert!(
        world.error.is_some()
            || world.named_values.get("always_fails") == Some(&"true".to_string())
    );
}
...
fn given_request_fails_after_retries(world: &mut TestWorld) {
    ...
    world
        .named_values
        .insert("retry_attempts".to_string(), "3".to_string());
}
...
fn when_fails_retryable(world: &mut TestWorld) {
    ...
    world
        .named_values
        .insert("is_retryable".to_string(), "true".to_string());
}
...
fn given_tx_unknown_status(world: &mut TestWorld) {
    world
        .named_values
        .insert("tx_status".to_string(), "unknown".to_string());
}
...
fn given_429_with_retry_after(world: &mut TestWorld, seconds: u64) {
    ...
    world
        .named_values
        .insert("retry_after_secs".to_string(), seconds.to_string());
}
...
fn given_retry_with_callback(world: &mut TestWorld) {
    ...
    world
        .named_values
        .insert("has_callback".to_string(), "true".to_string());
}
...
fn then_callback_invoked(world: &mut TestWorld) {
    assert_eq!(
        world.named_values.get("has_callback"),
        Some(&"true".to_string())
    );
}
