...
Given(
  "a known Ed25519 private key {string}",
...
Given(
  "a known Secp256k1 private key {string}",
  function (this: AptosWorld, hex: string) {
...
Given(
  "private key {string} from test vectors",
  function (this: AptosWorld, placeholder: string) {
    ...
    if (vectors && vectors.length > 0 && vectors[0].input?.private_key_hex) {
      ...
      this.testVectors.set(
        "expected_public_key",
        vectors[0].expected?.public_key_hex,
      );
    } else {
...
When(
  "I create an Ed25519 account from the private key",
  function (this: AptosWorld) {
    try {
      if (this.privateKey) {
...
When("I create an Ed25519 account", function (this: AptosWorld) {
  try {
    const pkHex =
      (this.testVectors.get("private_key") as string) || this.hexString;
    if (pkHex) {
      const pk = new Ed25519PrivateKey(pkHex);
      this.account = Account.fromPrivateKey({ privateKey: pk });
...
When(
  "I create a Secp256k1 account from the private key",
  function (this: AptosWorld) {
    try {
      this.account = Account.fromPrivateKey({
...
When(
  "I try to create an Ed25519 account from hex",
  function (this: AptosWorld) {
    try {
      const pk = new Ed25519PrivateKey(this.hexString!);
      this.account = Account.fromPrivateKey({ privateKey: pk });
...
Then("the addresses should be different", function (this: AptosWorld) {
  ...
  if (account1 && account2) {
    ...
    expect(first.accountAddress.toString()).to.not.equal(
...
Then(
  "recreating from the same key should produce the same address",
  function (this: AptosWorld) {
    if (this.privateKey) {
      ...
      expect(this.account!.accountAddress.toString()).to.equal(
        account2.accountAddress.toString(),
      );
    }
  },
...
Then(
  "the signature scheme should be {string}",
  function (this: AptosWorld, scheme: string) {
    ...
    if (expectedScheme === "ed25519") {
      ...
      expectedScheme === "secp256k1_ecdsa"
    ) {
...
Then(
  "the account address should be {string}",
...
Then("it should be {string}", function (this: AptosWorld, expected: string) {
  ...
  if (expected.toLowerCase() === "ed25519") {
    ...
    expect(String(this.result).toLowerCase()).to.include(
...
When(
  "I store both in a collection of Account references",
  function (this: AptosWorld) {
...
Then(
  "I should be able to iterate and sign with each",
  function (this: AptosWorld) {
...
Then("the address should match", function (this: AptosWorld) {
  ...
  expect(wrapped.accountAddress.equals(this.account!.accountAddress)).to.be
    .true;
});
...
When(
  "I create an AnyAccount based on the key type",
  function (this: AptosWorld) {
    try {
...
When(
  "I derive an Ed25519 account from the mnemonic",
  function (this: AptosWorld) {
    try {
      const mnemonic = this.testVectors.get("mnemonic") as string;
      const path =
        (this.testVectors.get("derivation_path") as string) ||
        "m/44'/637'/0'/0'/0'";
...
Then(
  "the derived address should match test vectors",
  function (this: AptosWorld) {
    const expected = this.testVectors.get("expected_address") as
      | string
      | undefined;
    if (expected) {
      const actual = this.account!.accountAddress.toString();
...
Then(
  "deriving again should produce the same account",
  function (this: AptosWorld) {
    const mnemonic = this.testVectors.get("mnemonic") as string;
    const path =
      (this.testVectors.get("derivation_path") as string) ||
