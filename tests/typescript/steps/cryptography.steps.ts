...
Given(
  "a valid 64-byte Ed25519 private key \\(seed + public key)",
...
Given(
  "a hex-encoded Ed25519 private key {string}",
...
Given(
  "a known Ed25519 private key from test vectors",
  function (this: AptosWorld) {
    ...
    if (keyVector) {
      ...
      this.testVectors.set(
        "expected_public_key",
        keyVector.expected.public_key_hex,
      );
    } else {
...
Given(
  "a known Ed25519 key pair from test vectors",
  function (this: AptosWorld) {
    const vectors = getSignatureVectors();
    const keyVector = vectors.ed25519?.key_vectors?.[0];
    if (keyVector) {
      ...
      this.testVectors.set(
        "expected_signature",
        keyVector.expected?.signature_hex,
      );
    } else {
...
When(
  "I create an Ed25519 key pair from the bytes",
  function (this: AptosWorld) {
    try {
      this.privateKey = new Ed25519PrivateKey(this.bytes!);
      this.publicKey = this.privateKey.publicKey().toUint8Array();
...
Then("the private keys should be different", function (this: AptosWorld) {
  ...
  expect(bytesToHex(pk1.toUint8Array())).to.not.equal(
    bytesToHex(pk2.toUint8Array()),
  );
});
...
Then(
  ...
  function (this: AptosWorld) {
    ...
    expect(bytesToHex(this.privateKey!.toUint8Array())).to.equal(
      bytesToHex(pk2.toUint8Array()),
    );
  },
);
...
Then(
  "the public key should match the embedded public key",
  function (this: AptosWorld) {
...
Then("the public key should match test vectors", function (this: AptosWorld) {
  ...
  const privateKeyPlaceholder = this.testVectors.get(
    "private_key_was_placeholder",
  );
...
Then(
  "the address should match the expected value from test vectors",
  function (this: AptosWorld) {
...
Then(
  "it should fail with an invalid private key error",
  function (this: AptosWorld) {
...
Given(
  "messages {string} and {string}",
  function (this: AptosWorld, msg1: string, msg2: string) {
...
Given("the message from test vectors", function (this: AptosWorld) {
  ...
  if (sigVector) {
    ...
    this.testVectors.set(
      "expected_signature",
      sigVector.expected.signature_hex,
    );
  } else {
...
Then(
  "the signature should be valid for the message",
  function (this: AptosWorld) {
...
Then("both signatures should be identical", function (this: AptosWorld) {
  ...
  expect(bytesToHex(sig1.toUint8Array())).to.equal(
    bytesToHex(sig2.toUint8Array()),
  );
});
...
Then("the signatures should be different", function (this: AptosWorld) {
  ...
  if (this.testVectors.has("signedTx1") && this.testVectors.has("signedTx2")) {
    ...
    tx1.serialize(s1);
    tx2.serialize(s2);
    expect(bytesToHex(s1.toUint8Array())).to.not.equal(
      bytesToHex(s2.toUint8Array()),
    );
  } else {
    const sig1 = this.testVectors.get("signature1") as Ed25519Signature;
    const sig2 = this.testVectors.get("signature2") as Ed25519Signature;
    expect(bytesToHex(sig1.toUint8Array())).to.not.equal(
      bytesToHex(sig2.toUint8Array()),
    );
  }
});
...
Then(
  ...
  function (this: AptosWorld) {
    ...
    if (expected) {
      const sig = this.result as Ed25519Signature;
      expect(bytesToHex(sig.toUint8Array()).toLowerCase()).to.equal(
...
Given(
  "a signature for message {string}",
...
When("I try to verify the signature", function (this: AptosWorld) {
  try {
    ...
    if (pk instanceof Ed25519PrivateKey) {
      const publicKey = pk.publicKey();
      const truncated = this.testVectors.get(
        "truncatedSignature",
      ) as Uint8Array;
      // This should throw
...
Then(
  "it should fail with an invalid signature error",
  function (this: AptosWorld) {
    expect(this.error).to.not.be.undefined;
  },
);
...
Then(
  "the result should start with {string}",
  function (this: AptosWorld, prefix: string) {
    expect(
      this.hexString!.startsWith(prefix) ||
        String(this.result).startsWith(prefix),
    ).to.be.true;
  },
);

Then(
  "the hex string should start with {string}",
  function (this: AptosWorld, prefix: string) {
...
Then(
  "the hex length should be 66 or 130 characters",
  function (this: AptosWorld) {
    // 32 bytes = 64 hex chars + 0x = 66, or 64 bytes = 128 hex chars + 0x = 130
    expect(this.hexString!.length === 66 || this.hexString!.length === 130).to
      .be.true;
  },
);
...
Then(
  "recreating from the bytes should produce the same key pair",
  function (this: AptosWorld) {
    const pk2 = new Ed25519PrivateKey(this.bytes!);
    expect(bytesToHex(this.publicKey!)).to.equal(
      bytesToHex(pk2.publicKey().toUint8Array()),
    );
  },
);
...
Then(
  "the private key bytes should not appear in the output",
  function (this: AptosWorld) {
...
When(
  "I create a Secp256k1 key pair from the bytes",
  function (this: AptosWorld) {
    try {
...
Then(
  ...
  function (this: AptosWorld) {
    ...
    expect(this.publicKey!.length === 33 || this.publicKey!.length === 65).to.be
      .true;
  },
);

Then(
  "the uncompressed public key should be 65 bytes",
  function (this: AptosWorld) {
...
Then(
  /^it should equal SHA3-256\(public_key \|\| 0x00\)$/,
  function (this: AptosWorld) {
...
Then(
  "it should equal the authentication key bytes",
  function (this: AptosWorld) {
    expect(bytesToHex(this.address!.toUint8Array())).to.equal(
